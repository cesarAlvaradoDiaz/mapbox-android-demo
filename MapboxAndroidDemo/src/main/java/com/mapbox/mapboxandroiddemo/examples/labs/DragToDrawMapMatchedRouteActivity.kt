...
class DragToDrawMapMatchedRouteActivity : AppCompatActivity() {
    private var desiredProfile = DirectionsCriteria.PROFILE_DRIVING
    ...
    private fun setUiAfterMapMatching() {
        Toast.makeText(this@DragToDrawMapMatchedRouteActivity,
                String.format(getString(R.string.move_map_explore_drawn_matched_route),
                        desiredProfile), Toast.LENGTH_SHORT).show()
        enableMapMovement()
    ...
    private fun initProfileSelectionFabs() {
        cycling_profile_fab.setOnClickListener {
            desiredProfile = DirectionsCriteria.PROFILE_CYCLING
            resetAllLists()
        }
        walking_profile_fab.setOnClickListener {
            desiredProfile = DirectionsCriteria.PROFILE_WALKING
            resetAllLists()
        }
        driving_profile_fab.setOnClickListener {
            desiredProfile = DirectionsCriteria.PROFILE_DRIVING
            resetAllLists()
        }
    }
    ...
    companion object {
        ...
        private const val DESIRED_MATCH_RADIUS = 45.0
    }
}